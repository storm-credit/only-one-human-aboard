...
## CHG-060 — v3 EP004 승인 (4 / 410) + 회차 분량 규정 정정
- Date: 2026-08-22
- Change Location:
  - `manuscript/v3/{_work,accepted}/act01/EP004.md` 신설 · `docs/manuscript/qa/V3-EP004-QA.md` 신설
  - `docs/manuscript/MANUSCRIPT-STATUS.md` 3/410 → **4/410** + **분량 규정 신설**
  - `docs/manuscript/MANUSCRIPT-PIPELINE-v1.md` **§2.1 분량 규정 신설**
  - `DEEP-V3-EP004` → `REALIZED`, `DEEP-V3-EP005` lane 2 주입
- **첫 비주인공 POV 화(미라).** 410화 중 91화가 비주인공 시점이라 구조적 시험대였다.
  판정: **감당한다.** 근거는 이전 불가능한 세 가지 — 문장 1의 단위 변환(시간 → 밀려나는 배송 칸),
  자기 부패 가능성에 대한 규칙(`"이름을 받으면 제가 더 착해진 척하게 됩니다."`),
  그리고 1장면에서 선언하고 마지막에 대가를 치르는 사적 경계(`"19:40 이후는 제 시간입니다."`).
- 집필 Codex / 심사 `episode-qa` **4회**. r1 REPAIR(P0×4·P1×9) → r2 REPAIR(9) → r3 REPAIR(1) → r4 PASS.
  **시리즈 최초로 정량 7종 전부 통과.**
- 무거운 것:
  - **종결 장치가 위장 반복이었다.** `대가`를 골랐다고 보고됐으나 EP001·002·004 모두
    `화면에 남은 미해결 값`으로 닫고 있었다. 대가가 *이름 지어졌을* 뿐 페이지에서 지불되지 않았다.
    → 마지막 이미지를 미라 자신의 산술로 교체(4,200명 × 17분).
  - **승인된 EP003과 불일치.** EP004가 3줄 알림을 보내는데 EP003에서 루카스는 2줄을 받았다.
    → 수신 목록 분리로 해결. 앞 두 줄은 EP003과 **글자 단위 일치** 재검증.
  - **미라가 관리자를 거절하는 대가가 없었다.** → 자기 3구역 재설계에 필요한 신뢰를 소모하게 함.
  - **M9 쌍둥이**: 북측 담당자와 터그 담당자가 동일 구문. → 터그 담당자를 평서 레지스터로 전환.
  - **숨은 패자 발생(D1).** M9 수정이 실제 사람을 만들었는데(`그 사람 어제도 일찍 나왔어요`)
    그 15분을 아무도 지불하지 않았다 — 하드 펜스 위반. → 초과 기록으로 처리.
  - **페이지에서 한 약속이 조용히 파기(D9).** → EP003의 기존 게시물에 연결해 종결.
- **일반화 가능한 발견**: r2에서 지적된 격언을 전부 잘랐는데 **수정 과정에서 3개가 새로 생겨 순증 0**이었다.
  r3에서 원인 확정 — *이 문체에서는 수정 중 추가되는 서술 문장이 기본적으로 격언 형태로 나온다.*
  r3의 추가 2건은 상한을 하나도 건드리지 않았고 이유가 구조적이다: 하나는 **대사**, 하나는 **완결된 행동**.
  → **상시 규칙: 수정 중에는 대사나 완결된 행동을 추가하라. 평가하는 서술 문장은 절대 추가하지 마라.**
    r2 = 실패 사례, r3 = 성공 사례로 배치 QA에 기록.
- **심사자가 자기 산술 오류를 잡았다.** r1에서 자기가 써 준 종결부 `사천 명 가까이`가 520×8=4,160과
  맞지 않았다. 2라운드를 그대로 통과시킨 뒤 스스로 신고 — *"산수를 끝까지 하고 결과를 숨기지 마라가
  주제인 화에서 주인공이 산수를 틀리면 비트가 뒤집힌다."* → `사천이백`.
- **분량 규정 정정 (작가 지적):** 오케스트레이터가 승인 3화에서 `5,410~5,539` 밴드를 **역산해 상한처럼
  사용**하고 있었다. 규정이 아니었다. 유일한 문서 규정은 v2 시대 `PROSE-EXECUTION-PROTOCOL-v1.1` §6
  (`5,500~8,000 / exact count is not Canon`)이며 v3 권위 목록에 없다.
  그 하한 기준으로 **EP001~003이 전부 5,500에 걸치거나 미달**(5,512/5,410/5,539)이었다 — 가짜 밴드가
  회차를 정당 범위 바닥에 묶고 있었고, 하마터면 주노 협상 장면을 "초과"로 잘라낼 뻔했다.
  → **하한 5,500 · 상한 없음. 초과는 결함이 아니다. 길이를 이유로 장면을 자르지 마라.**
- Canon / Blueprint / POV / Reveal 변경: **0**. v2 트리 변경: **0**. 무결성 410/410.
- Status: **`EP004 ACCEPTED / v3 4 of 410 / EP005 대기`**.
...
**`IN PROGRESS`** — active v3 **4 / 410** (EP001~004 accepted); historical v2 **10 / 230** frozen.
