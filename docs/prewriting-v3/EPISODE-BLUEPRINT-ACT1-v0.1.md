# V3 EPISODE BLUEPRINT — ACT 1 v0.1

Status: `ACTIVE EPISODE EXECUTION LINE / NO PROSE` — execution layer under v3 Canon (promoted 2026-08-21). Canon authority stays with the v3 Bibles; this layer is event/execution truth, not Canon.
Project: 《우주선에는 인간이 한 명뿐이다》
Act 1: **우리가 살던 메리디언 — EP001~042**

Authority:
- `canon/ACT_BIBLE-v3.2-CANDIDATE.md`
- `docs/design-v3/V3-410-EPISODE-DENSITY-MAP-v0.2.md`
- `docs/prewriting-v3/EPISODE-CARD-SPEC-v0.1.md`

Protagonist:
**루카스 켈러 / Lukas Keller** — reader-facing call-name `루카스` only (CHG-055, 2026-08-21).

## Act contract

Primary promise:
**주인공과 메리디언에 정들기 + 작은 실무 문제 해결의 재미 + 기록/물리 사이의 아주 작은 어긋남.**

Target protagonist-owned episodes:
**33 / 42 = 78.6%**.

Secondary-owned episodes:
- Mira 2
- Juno 2
- Selene 1
- Rafi 1
- Niko 1
- Gideon 1
- Transit Dispatcher / institutional 1

Strong cross-history autobiographical memory:
**0**.

No Act1 episode may objectively confirm regression, branching, substrate or HUMAN:1.

Chronology:
Several ordinary work/social weeks. Exact calendar remains C2; preserve enough breathing room that relationships feel habitual before Act2 begins.

---

# B01 EP001~007 — 일과 집

## EP001 — 오래된 진동
**POV:** 루카스
**Zone:** Old Works
**Immediate Want:** finish a routine structural-damage call without expanding the shutdown.
**A-Plot:** a freight-service support rail shows abnormal vibration and hairline cracking. He reconstructs load order, rejects a supervisor's easy `sensor fault` explanation, and narrows the physical cause.
**Relationship:** Gideon makes him justify each inference instead of endorsing it; Niko wants to crawl into a restricted access run too early.
**World:** establishes damage-analysis authority: he may assess structural safety, not command people or police access.
**Clue:** NONE. Ordinary failure.
**End Turn:** the immediate crack is real but not the original cause; investigation moves one layer upstream.
**Continuity Output:** Old Works diagnostics base, Gideon/Niko work triangle, protagonist method.

## EP002 — 원인은 한 칸 앞에 있다
**POV:** 루카스
**Zone:** Old Works service corridor
**Immediate Want:** locate the actual load source before operations resume.
**A-Plot:** traces repeated cargo shock through a miscalibrated transfer damper and proves the crack was consequence, not cause.
**Relationship:** Niko proposes a faster shortcut; protagonist refuses because access proof is insufficient.
**World:** shows repair crews, contractors/co-ops, signoff chain and ordinary schedule pressure.
**Clue:** NONE.
**End Turn:** repair can proceed, but freight timing must be changed, creating a small logistics knock-on.
**Continuity Output:** protagonist prefers causal reconstruction over quick patch.

## EP003 — 퇴근은 이동이다
**POV:** 루카스
**Zone:** Old Works → Transfer Belt → Midring home
**Immediate Want:** get home in time for a family obligation after shift delay.
**A-Plot:** ordinary commute is disrupted by the freight timing change from EP002; he navigates crowding and a missed connection.
**Relationship:** parents/sibling household introduced through ordinary message/call; no tragedy. He is late often enough that family complains without melodrama.
**World:** public transit, shift-change rush, convenience food, housing texture.
**Clue:** NONE.
**End Turn:** Mira's logistics office issues a reroute notice that affects his next-day access.
**Continuity Output:** home/work separation; family exists independently of mystery.

## EP004 — 한 노선 덜 쓰기
**POV:** Mira
**Zone:** Transfer Belt dispatch/allocation
**Immediate Want:** absorb an unplanned freight restriction without delaying medical and neighborhood deliveries.
**A-Plot:** she changes two schedules, refuses a manager's suggestion to dump all delay onto one residential service window, and negotiates an alternate route.
**Relationship:** brief independent contact with Juno over community delivery timing; protagonist is not part of the decision.
**World:** logistics authority and finite throughput; Mira has a career and pressure line unrelated to protagonist.
**Clue:** NONE.
**Knowledge Fence:** no hidden-history awareness.
**End Turn:** her reroute saves one obligation but shifts inconvenience toward Old Works commuter access.
**Continuity Output:** Mira↔Juno independent edge begins.

## EP005 — 남의 결정도 내 하루가 된다
**POV:** 루카스
**Zone:** Transfer Belt / Old Works
**Immediate Want:** reach a follow-up inspection despite changed route.
**A-Plot:** deals with Mira's reroute consequence, then sees why the allocation decision was rational even though it inconveniences him.
**Relationship:** first ordinary Mira encounter: old familiarity, easy irritation, no exposition dump about former relationship.
**World:** decisions propagate across departments without protagonist control.
**Clue:** NONE.
**End Turn:** new damage call arrives from a different Old Works bay.
**Continuity Output:** protagonist↔Mira current baseline is familiar but bounded.

## EP006 — 집에 있는 일
**POV:** 루카스
**Zone:** Midring
**Immediate Want:** keep a promised family visit rather than taking optional overtime.
**A-Plot:** chooses family over a noncritical call; sees sibling household/childcare or parent routine in normal use.
**Relationship:** family quietly establishes his habit of trying to fix practical problems for everyone; someone declines his help.
**World:** family, housing, ordinary food, childcare/elder routine.
**Clue:** NONE.
**End Turn:** work message says EP001 case passed retest without him.
**Continuity Output:** world works without protagonist; family boundaries seeded.

## EP007 — 고친 뒤의 하루
**POV:** 루카스
**Zone:** Old Works / Midring
**Immediate Want:** close the first case cleanly.
**A-Plot:** final verification confirms the repair. Nothing uncanny is hiding underneath it; closure paperwork also carries forward an old emergency-bracing checklist for the next safety renewal as routine inherited knowledge.
**Relationship:** Gideon credits Niko for one useful observation; protagonist realizes he over-controlled the junior.
**World:** closure paperwork, ordinary work satisfaction.
**Clue:** D05 mundane primer: an old emergency-bracing checklist exists as ordinary safety knowledge. No anomaly, no ominous framing; nobody finds it strange.
**End Turn:** a separate small industrial alarm breaks the routine.
**Continuity Output:** first practical payoff complete; ordinary failures are real.

---

# B02 EP008~014 — 평범한 사고

## EP008 — 경보는 이유가 있다
**POV:** 루카스
**Zone:** Old Works light-fabrication edge
**Immediate Want:** determine whether a localized industrial incident threatens adjacent structure.
**A-Plot:** a fabrication fixture fails, injuring workers and jamming a service barrier. Protagonist assesses secondary collapse risk.
**Relationship:** first active interface with emergency-response personnel.
**World:** ordinary workplace hazard, safety alarms, evacuation lanes.
**Clue:** NONE; MI-08 genuine accident.
**End Turn:** trapped worker cannot be reached through the obvious route.

## EP009 — 사고 순서
**POV:** 루카스
**Zone:** accident sector
**Immediate Want:** reconstruct failure order fast enough to choose a safe access path.
**A-Plot:** distinguishes heat damage from impact damage and identifies a service approach that remains structurally viable.
**Relationship:** Niko wants to accompany him; protagonist assigns him a safer measurement task.
**World:** technical competence produces options, not command authority.
**Clue:** NONE.
**End Turn:** response command refuses entry until its own hazard threshold is satisfied.

## EP010 — 문을 열 수 있다고 해서 여는 사람은 아니다
**POV:** 루카스
**Zone:** accident perimeter
**Immediate Want:** convince response command his access route is viable without exceeding his authority.
**A-Plot:** presents evidence rather than issuing an order; Selene challenges assumptions and imposes a narrower rescue window.
**Relationship:** protagonist↔Selene friction begins from jurisdiction, not personality insult.
**World:** emergency command boundary made concrete.
**Clue:** NONE.
**End Turn:** one injured worker's condition worsens before the window opens.

## EP011 — 안전선
**POV:** Selene
**Zone:** emergency command perimeter
**Immediate Want:** extract trapped workers without turning one local accident into a second casualty cluster.
**A-Plot:** balances protagonist's structural route, medical urgency and team exposure; rejects an emotionally appealing but unsafe shortcut.
**Relationship:** evaluates protagonist as useful but not command-safe if unchecked; coordinates Rafi.
**World:** crisis response operates without protagonist permission.
**Clue:** NONE.
**Knowledge Fence:** no hidden-system or protagonist specialness.
**End Turn:** authorizes a limited entry contingent on a field measurement protagonist must supply.

## EP012 — 제한된 길
**POV:** 루카스
**Zone:** damaged service access
**Immediate Want:** keep the rescue path within Selene's limit.
**A-Plot:** field measurement changes mid-entry; he aborts one route and uses the old bracing checklist to find a less direct brace point inside Selene's limit.
**Relationship:** accepts Selene's authority when she cuts off a tempting extension.
**World:** time and distance matter under damage.
**Clue:** D05 professional PLANT: one oddly specific inherited brace step proves useful under live damage. Noticed for professional reasons only; no origin claim, no mystery music.
**End Turn:** Rafi chooses to stay with one trapped casualty during extraction.

## EP013 — 한 명 더
**POV:** Rafi
**Zone:** damaged work cell
**Immediate Want:** stabilize one worker long enough for extraction.
**A-Plot:** refuses to abandon a salvageable casualty but stays within a bounded rescue plan; another casualty must be triaged elsewhere.
**Relationship:** first clear Rafi↔Selene trust line; he argues, she gives a narrow extension.
**World:** medical rescue has its own competence and cost.
**Clue:** NONE.
**Knowledge Fence:** no mystery role.
**End Turn:** rescue succeeds, but the last failure evidence is about to be destroyed by cleanup.

## EP014 — 사고는 사고였다
**POV:** 루카스
**Zone:** Old Works / post-incident review
**Immediate Want:** close cause before cleanup erases evidence.
**A-Plot:** confirms an ordinary wear + procedural timing failure, not sabotage or hidden-system interference; the one unresolved note is administrative — the brace step that helped has no matching origin incident under current identifiers.
**Relationship:** professional respect with Selene/Rafi improves; protagonist admits his first route would have been worse.
**World:** incident review and worker consequences.
**Clue:** PAYOFF of ordinary-accident expectation + D05 **local bounded reward** (not a ledger payoff): the accident was exactly what it looked like, and one useful inherited procedure still has no local origin. D05 `PARTIAL/FINAL PAYOFF` remains untouched in its Act7 window.
**End Turn:** after-shift family/community commitment pulls him away from work.
**Continuity Output:** teaches reader cause diversity.

---

# B03 EP015~021 — 다른 사람들의 일상

## EP015 — 저녁 자리
**POV:** 루카스
**Zone:** Midring / Juno venue
**Immediate Want:** eat, keep a social promise, avoid talking about work all evening.
**A-Plot:** community venue faces a delivery delay and a small dispute over reservation/food allocation.
**Relationship:** Juno introduced through competence with people, not as plot helper; Mira's work decision is discussed without her presence.
**World:** food, local business/co-op rhythm, repeat customers.
**Clue:** NONE.
**End Turn:** Juno decides to challenge the allocation process directly rather than asking protagonist to intervene.

## EP016 — 해결해 주지 않는 일
**POV:** 루카스
**Zone:** Midring
**Immediate Want:** help Juno with the logistics problem.
**A-Plot:** Juno refuses his attempt to mediate; he lacks authority and would complicate it.
**Relationship:** protagonist learns a small boundary; old familiarity with Mira makes him assume access he does not have.
**World:** civic complaint/co-op channels exist.
**Clue:** NONE.
**End Turn:** Juno goes to the actual allocation channel.

## EP017 — 누구부터 먹일 것인가
**POV:** Juno
**Zone:** Midring community/co-op
**Immediate Want:** keep regular service running while honoring a larger event commitment and unexpected outsider demand.
**A-Plot:** chooses a compromise that annoys both insiders and outsiders but keeps the venue open and avoids hoarding.
**Relationship:** Juno↔Mira conflict becomes direct: community fairness vs network optimization.
**World:** community network has independent goals and social power.
**Clue:** NONE.
**End Turn:** Mira offers a different supply window with a condition Juno dislikes.

## EP018 — 가족에게는 사건이 아니다
**POV:** 루카스
**Zone:** Midring family neighborhood
**Immediate Want:** handle an ordinary sibling/parent request after work.
**A-Plot:** practical household problem—repair appointment, school pickup, or housing-office issue—forces him to live outside professional competence.
**Relationship:** family member makes a decision without him; protagonist's instinct to take over is gently resisted.
**World:** school/housing bureaucracy and family routine.
**Clue:** NONE.
**End Turn:** he learns Mira/Juno have already settled their dispute.

## EP019 — 배분표 밖의 사람들
**POV:** Mira
**Zone:** Transfer Belt / Midring interface
**Immediate Want:** preserve network stability without treating Juno's community as an abstract line item.
**A-Plot:** revises timing based on local information but refuses a permanent exception that would privilege Juno's network.
**Relationship:** Mira↔Juno gains respect without friendship shortcut; protagonist never enters the negotiation.
**World:** formal logistics and informal community both matter.
**Clue:** NONE.
**End Turn:** the new arrangement creates a minor commute pattern protagonist notices later.

## EP020 — 이미 끝난 결정
**POV:** 루카스
**Zone:** Transfer Belt / Midring
**Immediate Want:** understand why his route changed again.
**A-Plot:** encounters the consequences of a conflict that resolved without him.
**Relationship:** Mira refuses his casual attempt to frame the solution as something he would have done differently; old relationship boundary sharpens slightly.
**World:** offscreen causality becomes visible through ordinary consequence.
**Clue:** NONE.
**End Turn:** Niko invites him to a leisure/community event he normally skips.

## EP021 — 일하지 않는 시간
**POV:** 루카스
**Zone:** Garden Commons / leisure venue
**Immediate Want:** spend a few hours not working.
**A-Plot:** sport/club/community event; a tiny practical mishap is solved by others before he can take over.
**Relationship:** Niko's independent social life and peer network become visible; protagonist is not mentor every minute.
**World:** leisure, public green space, young-adult culture.
**Clue:** NONE or purely mundane false oddity.
**End Turn:** next workday assigns protagonist to review another old safety procedure.

---

# B04 EP022~028 — 아주 작은 어긋남

## EP022 — 출처 없는 절차
**POV:** 루카스
**Zone:** Old Works diagnostics base
**Immediate Want:** validate an old emergency-bracing procedure before renewal.
**A-Plot:** a second old emergency-bracing procedure is technically effective, but its origin case cannot be found in the accessible archive either.
**Relationship:** Gideon tells him undocumented institutional knowledge is not automatically conspiracy.
**World:** safety knowledge can outlive paperwork.
**Clue:** D05 Knowledge Without History — PLANT (second procedure family). Ledger allows `one or two procedures` inside the Act1~3 PLANT window; this is the second, not a REINFORCE.
**End Turn:** this procedure contains a different oddly specific step that seems excessive.

## EP023 — 너무 조심스러운 규칙
**POV:** 루카스
**Zone:** Old Works test bay
**Immediate Want:** determine whether the odd step is obsolete bureaucracy.
**A-Plot:** controlled test shows the extra step prevents a rare secondary failure but costs time/resources.
**Relationship:** Niko questions why they keep rules nobody can explain.
**World:** institutional conservatism can be useful and inefficient.
**Clue:** D05 REINFORCE, still mundane explanations dominate.
**End Turn:** archive search returns no originating incident under current identifiers.

## EP024 — 기록에 없다고 없던 일은 아니다
**POV:** 루카스
**Zone:** local records / Old Works
**Immediate Want:** close the procedure review without inventing a mystery.
**A-Plot:** finds partial provenance through renamed departments but still no incident matching the precise lesson.
**Relationship:** Gideon pushes him to separate `unknown` from `impossible`.
**World:** bureaucracy changes names and loses context naturally.
**Clue:** D05 stays OPEN, no payoff.
**End Turn:** procedure is renewed provisionally.

## EP025 — 다른 길을 배우는 사람
**POV:** 루카스
**Zone:** Garden Commons / training facility
**Immediate Want:** keep Niko on a conservative qualification path.
**A-Plot:** watches Niko perform a practical exercise with peers; Niko is more competent outside protagonist's supervision than expected.
**Relationship:** protection/control tension increases.
**World:** apprenticeship/training culture.
**Clue:** NONE.
**End Turn:** Niko quietly applies for independent field qualification.

## EP026 — 내 이름으로 신청한다
**POV:** Niko
**Zone:** training/admin route
**Immediate Want:** submit an independent qualification request without asking protagonist to approve his adulthood.
**A-Plot:** he seeks Gideon/administrative signoff through normal channels, accepts one failed module and reschedules rather than cheating.
**Relationship:** Niko↔Gideon independent mentorship edge strengthens.
**World:** career mobility and certification.
**Clue:** NONE.
**Knowledge Fence:** no hidden-history awareness.
**End Turn:** protagonist learns only after the application is already filed.

## EP027 — 허락하지 않아도 되는 일
**POV:** 루카스
**Zone:** Old Works
**Immediate Want:** persuade Niko to withdraw or delay.
**A-Plot:** realizes he has no authority to veto a valid application; shifts from prohibition to demanding preparation.
**Relationship:** durable protagonist↔Niko boundary change.
**World:** institutions do not grant protagonist paternal control.
**Clue:** NONE.
**End Turn:** a real field case becomes Niko's next supervised test.

## EP028 — 이상한 절차, 평범한 결과
**POV:** 루카스
**Zone:** Old Works field site
**Immediate Want:** use the renewed safety procedure during a mundane repair.
**A-Plot:** the procedure works and prevents a small secondary fault; the job closes normally.
**Relationship:** protagonist gives Niko a real responsibility and does not take it back.
**World:** ordinary competence payoff.
**Clue:** D05 **second local bounded reward** (not a ledger payoff): useful knowledge repeats across separate procedure families; origins still unknown. D05 `PARTIAL/FINAL PAYOFF` remains untouched in its Act7 window.
**End Turn:** inspection materials from another site show a version/wear mismatch harder to explain bureaucratically.

---

# B05 EP029~035 — 물리적으로 맞지 않는 것

## EP029 — 재료번호
**POV:** 루카스
**Zone:** Old Works structural retrofit corridor
**Immediate Want:** certify a repair layer for continued use.
**A-Plot:** material generation and documented renovation date do not line up cleanly.
**Relationship:** Niko notices the mismatch first and earns credit.
**World:** material provenance matters to safety.
**Clue:** D03 Impossible Infrastructure — PLANT/REINFORCE.
**End Turn:** paperwork says the impossible layer should never have been installed there.

## EP030 — 오래된 새것
**POV:** 루카스
**Zone:** Old Works
**Immediate Want:** decide whether the mismatch is counterfeit material, mislabeling or unsafe hidden repair.
**A-Plot:** wear pattern indicates the layer has been present longer than the recorded retrofit.
**Relationship:** protagonist requests Gideon's independent review instead of protecting his own theory.
**World:** safety review can shut down production access.
**Clue:** D03 REINFORCE.
**End Turn:** Gideon takes the evidence away rather than agreeing immediately.

## EP031 — 두 번째 눈
**POV:** Gideon
**Zone:** diagnostics lab / records
**Immediate Want:** falsify the protagonist's conclusion before escalating a disruptive shutdown.
**A-Plot:** checks calibration, chain of custody, known retrofit exceptions and record migrations; several mundane explanations fail.
**Relationship:** Gideon treats Niko's observation seriously; his caution is active work, not passivity.
**World:** independent technical review.
**Clue:** D03 REINFORCE, still no ontology inference.
**Knowledge Fence:** Gideon does not secretly know system truth.
**End Turn:** one old identifier points to a retired access registry.

## EP032 — 닫힌 번호
**POV:** 루카스
**Zone:** Old Works / local archive interface
**Immediate Want:** map the retired identifier to a physical maintenance route.
**A-Plot:** identifier resolves to a decommissioned service classification but not a current route.
**Relationship:** Sora/security compliance enters only to authorize restricted records, not join a mystery team.
**World:** access logging/privacy/safety boundaries.
**Clue:** D03 + future Uncountable Index very soft seed; no scale meaning.
**End Turn:** route map and material location overlap imperfectly.

## EP033 — 막아 두는 이유
**POV:** 루카스
**Zone:** restricted Old Works access
**Immediate Want:** inspect only the physically relevant section.
**A-Plot:** finds a mundane reason much of the area is closed—obsolete pressure/fire isolation—plus one repair seam whose chronology still does not fit.
**Relationship:** accepts Sora's restriction instead of treating security as obstruction.
**World:** most legacy space is ordinary infrastructure history.
**Clue:** D03 REINFORCE; avoids secret-dungeon trope.
**End Turn:** transit maintenance outside the case forces supplies to reroute.

## EP034 — 우회하는 하루
**POV:** Juno
**Zone:** Midring / Transfer interface
**Immediate Want:** keep ordinary community service running through a transit maintenance day.
**A-Plot:** works around a reroute, deals with delivery riders/parents/shift workers, and interacts briefly with a dispatcher whose choices affect many people.
**Relationship:** Juno↔Mira network familiarity grows independently.
**World:** seeds Transit Dispatcher as ordinary worker before later importance.
**Clue:** NONE.
**End Turn:** a delayed maintenance crew arrives at Old Works after protagonist's team has already changed its work plan.

## EP035 — 모르는 채로 닫기
**POV:** 루카스
**Zone:** Old Works
**Immediate Want:** make a safety decision despite incomplete historical explanation.
**A-Plot:** certifies a limited repair/use restriction based on current physical condition, explicitly separating safety judgment from historical truth.
**Relationship:** Gideon approves the discipline; Niko dislikes leaving the mystery open.
**World:** practical work can close without cosmology answer.
**Clue:** D03 remains OPEN.
**End Turn:** routine trend review flags a cooling segment drifting outside its expected band.

---

# B06 EP036~042 — 다음 고장의 씨앗

## EP036 — 조금 높은 온도
**POV:** 루카스
**Zone:** Old Works / cooling distribution edge
**Immediate Want:** determine whether a small thermal drift requires shutdown.
**A-Plot:** drift is within emergency tolerance but outside long-term trend; orders additional inspection, not alarm.
**Relationship:** Niko pushes for immediate invasive check; protagonist chooses proportionate response.
**World:** maintenance backlog and risk prioritization.
**Clue:** Act2 cooling failure — PLANT.
**End Turn:** one inspection slot must be moved because of access scheduling.

## EP037 — 같은 번호가 두 번
**POV:** 루카스
**Zone:** diagnostics/asset registry
**Immediate Want:** clear an asset-identification conflict before inspection.
**A-Plot:** two legacy references partially collide around the cooling corridor; operations treats it as a migration artifact.
**Relationship:** Gideon insists on documenting rather than `cleaning` the oddity away.
**World:** legacy asset bureaucracy.
**Clue:** duplicate legacy ID — PLANT; very soft future namespace clue.
**End Turn:** maintenance scheduler chooses which physical inspection moves first.

## EP038 — 미룬 점검
**POV:** 루카스
**Zone:** Old Works planning / field
**Immediate Want:** protect the higher-risk inspection slot.
**A-Plot:** scheduler shifts one low-priority segment, not the critical measurement, due labor/access collision. Decision is reasonable, not villainous negligence.
**Relationship:** protagonist argues but accepts the final schedule after risk review.
**World:** ordinary institutional compromise can later matter without being sabotage.
**Clue:** Act2 schedule chain — PLANT.
**End Turn:** Transfer Belt announces route maintenance overlapping the new inspection window.

## EP039 — 노선을 하나 비운다
**POV:** Transit Dispatcher / institutional lens
**Zone:** Transfer Belt control
**Immediate Want:** free a service route for planned maintenance without damaging peak passenger flow.
**A-Plot:** moves one service pattern and creates a temporary fallback route through the future Act2 boundary sector.
**Relationship:** ordinary coordination with Mira's network; no protagonist contact required.
**World:** establishes dispatcher's visible competence/access before any future cascade importance.
**Clue:** Act2 geography setup — PLANT, not mystery.
**Knowledge Fence:** entirely ordinary operational reasoning.
**End Turn:** fallback route is approved for the coming maintenance window.

## EP040 — 연습으로 닫는 문
**POV:** 루카스
**Zone:** Midring school/community boundary near Old Works-Transfer interface
**Immediate Want:** certify structural/access conditions for a routine evacuation drill.
**A-Plot:** school/caregiver staff run a drill; protagonist checks doors/routes rather than leading children.
**Relationship:** caregiver/parent role seeded; protagonist sees how civilians interpret technical instructions differently.
**World:** emergency literacy is ordinary civic life.
**Clue:** Act2 evacuation/caregiver seed — PLANT.
**End Turn:** one bulkhead behavior triggers a brief Arun association.

## EP041 — 기억한 말
**POV:** 루카스
**Zone:** Old Works / old bulkhead route
**Immediate Want:** finish drill follow-up without dwelling on Arun.
**A-Plot:** a mundane door-sequence resembles the old accident. He recalls one specific phrase/sequence from Arun that does not cleanly match an old accessible log, but attributes it to trauma/time.
**Relationship:** Gideon notices discomfort but does not force confession; Niko only knows the broad accident history.
**World:** old accidents leave human habits.
**Clue:** Arun autobiographical mismatch — extremely soft PLANT. NOT proof of cross-history memory.
**End Turn:** cooling trend update shows the drift stopped for now.

## EP042 — 평범하게 끝난 주
**POV:** 루카스
**Zone:** Old Works → Transfer Belt → Midring/Juno venue
**Immediate Want:** close open work items and return to ordinary life.
**A-Plot:** current cooling check passes within tolerance; duplicate ID is logged for later cleanup; no disaster happens yet.
**Relationship:** short ordinary contact with Mira/family/Juno/Niko confirms recurring relationships before they are threatened.
**World:** Meridian feels inhabited and functional.
**Clue:** cooling/legacy-ID/Arun plants remain dormant; no new clue.
**End Turn:** next scheduled maintenance window is already on the calendar.
**Continuity Output:** Act2 ingredients are fair-planted without ominous author narration.

---

# ACT 1 QA SUMMARY

## POV count
- 루카스: **33**
- Mira: 2
- Juno: 2
- Selene: 1
- Rafi: 1
- Niko: 1
- Gideon: 1
- Transit Dispatcher/institutional: 1
- Total: 42

Target match:
**33/42 = 78.6% protagonist-centered — PASS.**

## Practical payoff count
At least:
- EP001~007 crack/damper case
- EP008~014 industrial accident
- EP022~028 procedure qualification/current repair
- EP029~035 material-mismatch safety decision
- EP036~042 cooling trend currently bounded

PASS: Act1 is not 42 episodes of setup.

## Ordinary-world anchors seeded
- protagonist home block
- family household
- Old Works diagnostics base
- Transfer Belt commute/dispatch
- Juno community/food venue
- Garden Commons leisure
- school/caregiver route
- emergency/medical interface

PASS.

## Relationship durable movements
- protagonist↔Mira: familiarity + boundary
- Mira↔Juno: independent working relationship
- protagonist↔Niko: control → limited autonomy
- protagonist↔Selene/Rafi: jurisdictional respect
- Gideon↔Niko: independent mentorship seed
- protagonist↔family: help/control boundary

PASS.

## Reveal fence
Act1 contains:
- D05 Knowledge Without History soft seed
- D03 Impossible Infrastructure material chronology seed
- Act2 cooling/legacy-ID/route/caregiver seeds
- one tiny Arun mismatch

It does NOT contain:
- explicit cross-history memory claim
- HUMAN:1
- substrate vocabulary
- regression theory
- prophet community
- author confirmation of alternate histories

PASS.

## Cut risk
Most compressible cluster remains EP015~021 or EP022~028 if prose repetition appears.
Do not cut the independent Mira↔Juno resolution or Niko autonomy movement; those prove living-world causality.

## Current verdict
**ACT1 EP001~042 BLUEPRINT v0.1 = PREWRITING PASS CANDIDATE / requires independent Act1 hostile QA before lock.**