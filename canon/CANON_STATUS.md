...
이 문서는 브레인스토밍 결과가 조용히 정본으로 승격되는 것을 막는다.

상태:
- `CANON` — Freeze된 정본
- `PROVISIONAL` — 현재 1순위이나 변경 가능
- `CANDIDATE` — 비교 중
- `REJECTED` — 폐기

## Operational Rules — CANON

| Item | Status | Note |
|---|---|---|
| 설계 Freeze 전 원고 집필 금지 | CANON | 프롤로그/1화/샘플 포함 |
| 주요 설계는 비교 후 선택 | CANON | 필요 시 3~4안 |
| Phase 종료 시 Red Team | CANON | P0는 진행 차단 |
| 변경 시 영향 범위 기록 | CANON | 캐릭터/Act/복선/세계규칙 |
| 20화 Reader Promise를 100/200화에도 유지 | CANON | 장기성 핵심 |
| 인간 선 / AI 악 이분법 금지 | CANON | 기본 금지사항 |
| 하나의 반전만 장기 미루기 금지 | CANON | Reveal이 새 행동을 열어야 함 |

## Premise

| Item | Status | Note |
|---|---|---|
| 세대우주선 | PROVISIONAL | 정확 구조 미확정 |
| 인구 약 30만 | PROVISIONAL | 수용능력 Band 모델 검증 중 |
| 현재 자연기원 인간 의식은 한 명 | PROVISIONAL | 기술적 정의는 ‘생물학적 몸 1명’보다 복잡함 |
...
6. Red Team 통과
7. 변경 영향 기록 완료
8. 관련 World Logic/Act/복선 문서 동기화
